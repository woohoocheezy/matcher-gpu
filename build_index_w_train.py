...
# --- 👈 [수정] 메모리 및 GPU 부하 조절을 위한 설정값 변경 ---
BATCH_SIZE = 100000
TRAIN_SAMPLE_SIZE = 250000  # 훈련 샘플 수를 100만 -> 25만으로 줄여 VRAM 부담 감소
...
async def main():
    ...
    print("Connecting to DB..."); conn = await asyncpg.connect(**DB_CONFIG); print("  -> DB 연결 성공")
    ...
    print(f"  -> 훈련용 데이터 가져오기 완료 ({len(train_records)}개, 소요 시간: {time.time() - query_start_time:.2f}초)")

    if len(train_records) < 1000:
        print("훈련용 데이터 샘플이 너무 적습니다 (1000개 미만). 종료합니다.")
        await conn.close()
        return
    ...
    # 👈 [수정] nlist 값을 보수적으로 설정하여 VRAM 사용량 줄이기
    # 데이터 양이 많아도 nlist가 너무 커지지 않도록 최대값을 4096 정도로 제한
    nlist = min(4096, int(np.sqrt(len(train_records))))
    print(f"  -> 클러스터(nlist) 개수: {nlist}")
    ...
    async with conn.transaction():
        async for record in conn.cursor(main_query, start_date, end_date):
            if len(record['face_descriptor']) == VECTOR_DIMENSION:
                ...
                vector = np.frombuffer(record['face_descriptor'], dtype=np.uint8).astype('float32').reshape(1, -1)
