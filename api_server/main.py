...
async def search_face(request: Request, data: SearchRequest):
    ...
    try:
        ...
        resp = await request.app.state.http_client.post(
            FAISS_SERVER_URL,
            json={"descriptor_hex": data.descriptor_hex, "top_k": 100},
            timeout=10.0,
        )
        ...
        details = await fetch_details(
            request.app.state.db_pool, data.org_group_id, matched_ids
        )
        ...
        for an_id in matched_ids:
            ...
            if detail and start_naive <= detail["created_at"] < end_naive:
                if detail.get("face_image_path"):
                    detail["face_image_path"] = (
                        f"{IMAGE_STORE_URL}{detail['face_image_path']}"
                    )
                if detail.get("body_image_path"):
                    detail["body_image_path"] = (
                        f"{IMAGE_STORE_URL}{detail['body_image_path']}"
                    )
                final_results.append(detail)

        print(
            f"Total request processed in {(time.time() - overall_start_time) * 1000:.2f} ms."
        )
